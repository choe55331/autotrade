...
def get_chart_data(stock_code: str):
    ...
    try:
        ...
        if not _bot_instance:
            print(f"[ERROR] bot_instance is None")
            return jsonify({
                'success': False,
                ...
                if _realtime_chart_manager:
                    try:
                        if stock_code in _realtime_chart_manager.charts:
                            ...
                            if candle_count > 0:
                                print(f"[OK] Using real-time minute data ({candle_count} candles)")
                            ...
                            try:
                                ...
                                if success:
                                    print(f"[OK] {stock_code} added to real-time tracking")
                                    minutes = int(timeframe) if timeframe == '1' else 60
                                    daily_data = _realtime_chart_manager.get_minute_data(stock_code, minutes=minutes)
                                ...
                                print(f"WARNING: Failed to add stock to real-time tracking: {e}")
                    except Exception as e:
                        print(f"WARNING: Real-time data fetch failed: {e}")
                    ...
                    if hasattr(_bot_instance.data_fetcher, 'get_minute_price'):
                        ...
                        """
                            ...
                            if not daily_data or len(daily_data) == 0:
                                print(f"WARNING: {timeframe}-minute data not available (likely weekend/holiday), falling back to daily data")
                                actual_timeframe = 'D'
                            ...
                            print(f"WARNING: Minute data fetch failed ({e}), falling back to daily data")
                            actual_timeframe = 'D'
                            daily_data = _bot_instance.data_fetcher.get_daily_price(
                                stock_code=stock_code,
                                start_date=start_date_str,
                        ...
                        print(f"WARNING: Minute price method not available, using daily data")
                        actual_timeframe = 'D'
                        daily_data = _bot_instance.data_fetcher.get_daily_price(
                            stock_code=stock_code,
                            start_date=start_date_str,
...
"""
                ...
                print(f"[OK] Chart data ready: {len(chart_data)} points")
                if len(chart_data) > 0:
                    print(f" Date range: {chart_data[0]['time']} to {chart_data[-1]['time']}")
            else:
                print(f"WARNING: No daily data received from API")
            ...
            error_msg = str(e)
            print(f"[ERROR] Chart data fetch error for {stock_code}: {error_msg}")
            import traceback
            traceback.print_exc()
...
"""
        ...
        print(f"[ERROR] Volume rank API error: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e), 'data': []})


@market_bp.route('/api/market/price-change-rank')
...
"""
        ...
        print(f"[ERROR] Price change rank API error: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e), 'data': []})


@market_bp.route('/api/market/trading-value-rank')
...
"""
        ...
        print(f"[ERROR] Trading value rank API error: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e), 'data': []})
