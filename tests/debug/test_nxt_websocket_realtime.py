...
async def test_websocket_realtime():
    ...
    print(f"\n{BLUE}{'='*100}{RESET}")
    print(f"{BLUE}[SEARCH] WebSocket 실시간 가격 모니터링 (KRX + NXT 혼합){RESET}")
    ...
    if is_regular_hours:
        ...
        print(f"\n{YELLOW}[WARNING]️  현재 시각 {now.strftime('%H:%M')} - 정규장 종료{RESET}")
    ...
    try:
        ...
        print(f"{GREEN}[OK] REST API 연결 성공{RESET}")
        ...
        print(f"{GREEN}[OK] WebSocket 연결 성공{RESET}")
                ...
                for idx, item in enumerate(data_list):
                    ...
                    if base_code in price_history:
                        ...
                        try:
                            ...
                            received_count[0] += 1
                            print(f"    {GREEN}[OK] 저장 성공: {price_history[base_code]['name']} = {cur_prc:,}원{RESET}")

                        except Exception as e:
                            print(f"    {RED}[X] 파싱 실패: {e}{RESET}")
                    else:
                        print(f"    {YELLOW}[WARNING]️  종목코드 '{base_code}' 매칭 실패{RESET}")
        ...
        if not success:
            print(f"{RED}[X] 구독 실패{RESET}")
            return

        print(f"{GREEN}[OK] 구독 성공!{RESET}")
        ...
        print(f"\n{MAGENTA}{'='*100}{RESET}")
        print(f"{MAGENTA}[CHART] 실시간 데이터 수신 모니터링 (10회, 5초 간격){RESET}")
        ...
        for round_num in range(1, 11):
            ...
            if stocks_with_data == 0:
                print(f"  {YELLOW}[WARNING]️  아직 데이터 수신 없음...{RESET}")
        ...
        for code, data in price_history.items():
            ...
            print(f"  [CHART] 수신 횟수: {len(prices)}회")
        ...
        if stocks_with_data == 0:
            print(f"\n{RED}[X] WebSocket 실시간 데이터 수신 완전 실패{RESET}")
            print(f"{YELLOW}가능한 원인:{RESET}")
            print(f"  1. WebSocket 연결 문제")
            print(f"  2. 구독 타입(0B) 문제")
            print(f"  3. 토큰 권한 문제")
        elif krx_with_data == 0 and nxt_with_data == 0:
            print(f"\n{RED}[X] KRX/NXT 모두 데이터 수신 실패{RESET}")
        elif krx_with_data > 0 and nxt_with_data == 0:
            print(f"\n{YELLOW}[WARNING]️  KRX만 데이터 수신, NXT 데이터 수신 실패{RESET}")
            ...
            if krx_with_change > 0 and nxt_with_change > 0:
                ...
                print(f"\n{YELLOW}[WARNING]️  KRX는 변동 있으나 NXT는 변동 없음{RESET}")
                print(f"{YELLOW}   -> WebSocket은 작동하지만 NXT 거래량 부족일 가능성{RESET}")
            elif krx_with_change == 0 and nxt_with_change > 0:
                print(f"\n{YELLOW}[WARNING]️  NXT는 변동 있으나 KRX는 변동 없음 (예상 외){RESET}")
            else:
                print(f"\n{YELLOW}[WARNING]️  데이터 수신은 됐으나 가격 변동 없음{RESET}")
