        ...
        if response.status_code == 200:
            ...
            if endpoint == '/api/account':
                if data.get('total_assets', 0) == 0 and data.get('cash', 0) == 0:
                    print(f"\n[WARNING]️  경고: 모든 값이 0입니다. 하드코딩이거나 데이터가 없을 수 있습니다.")
                else:
                    print(f"\n[OK] 실제 데이터 확인됨!")
                ...
                print(f"\n연결 상태:")
                print(f"   - REST API: {'[OK] 연결됨' if data.get('rest_api') else '[X] 끊김'}")
                print(f"   - WebSocket: {'[OK] 연결됨' if data.get('websocket') else '[X] 끊김'}")
                print(f"   - Gemini AI: {'[OK] 연결됨' if data.get('gemini') else '[X] 끊김'}")
                print(f"   - 테스트 모드: {'🧪 활성' if data.get('test_mode') else '⚡ 비활성'}")
                print(f"   - Database: {'[OK] 연결됨' if data.get('database') else '[X] 끊김'}")

            return True

        else:
            print(f"[X] 실패! (HTTP {response.status_code})")
            print(f"응답: {response.text}")
            return False

    except requests.exceptions.ConnectionError:
        print(f"[X] 연결 실패! 대시보드가 실행 중인지 확인하세요.")
...
[WARNING]️  주의: 이 스크립트를 실행하기 전에 먼저 다음 명령어로 봇을 실행하세요:
...
"""
    ...
    for endpoint, success in results:
        status = "[OK] 통과" if success else "[X] 실패"
    ...
    if passed == total:
        ...
        print(f"\n[WARNING]️  일부 API 엔드포인트에 문제가 있습니다.")
