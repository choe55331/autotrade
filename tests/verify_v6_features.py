...
def test_virtual_trading_data_enricher():
    ...
    try:
        ...
        print(f"\n[CHART] 원본 필드: {len(test_stock_data)}개")
        print(f"[UP] Enriched 필드: {len(enriched)}개")
...
def test_virtual_trading_strategies():
    ...
    try:
        ...
        print(f"[OK] {len(strategies)}개 전략 로드 성공\n")
        ...
        for name, strategy in strategies.items():
            try:
                ...
                print(f"  [OK] {name}: {results[name]['signal']}")
            except Exception as e:
                results[name] = {
                    'status': '[X] 에러',
                    'error': str(e)
        ...
        working = sum(1 for r in results.values() if r['status'] == '[OK] 작동')
        print(f"\n[CHART] 결과: {working}/{len(strategies)} 전략 작동 중")

        if working == len(strategies):
            print("🎉 12개 전략 모두 정상 작동!")
            return True
        else:
            print(f"[WARNING]️ {len(strategies) - working}개 전략에 문제 있음")
            return False
...
def test_unified_ai_analyzer():
    ...
    try:
        ...
        print(f"[OK] Analyzer 초기화 성공")
        print(f"[CHART] 사용 가능한 Provider: {list(analyzer.providers.keys())}")
        print(f"[TARGET] 기본 Provider: {analyzer.default_provider}")

        if not analyzer.providers:
            print("[WARNING]️ 사용 가능한 AI Provider 없음 (API 키 확인 필요)")
            return False
...
def test_unified_risk_manager():
    ...
    try:
        ...
        print(f"[OK] Risk Manager 초기화 성공 (모드: balanced)")
...
def test_batch_api_client():
    ...
    try:
        ...
        print(f"[OK] Batch Client 초기화 (배치크기: 10, workers: 5)")

        test_codes = [f"{i:06d}" for i in range(100)]

        print(f"\n[CHART] 100개 종목 가격 조회 테스트:")
...
def test_redis_cache():
    ...
    try:
        ...
        if cached == test_value:
            print(f"[OK] 캐시 조회 성공")
            print(f"[CHART] Redis 연결: {cache_manager.redis_available}")
            ...
            print(f"[X] 캐시 데이터 불일치")
            return False

    except Exception as e:
        print(f"[X] 테스트 실패: {e}")
        print(f"[WARNING]️ Redis가 없어도 Memory fallback으로 작동합니다")
        return True
...
def main():
    ...
    if passed == total:
        ...
        print(f"[WARNING]️ {total - passed}개 기능에 문제가 있습니다.")
